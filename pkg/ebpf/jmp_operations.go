...
func (c *IMMJMPOperation) GenerateBytecode() []uint64 {
	bytecode := []uint64{encodeImmediateJmpOperation(c.Instruction, c.InsClass, c.DstReg.RegisterNumber(), c.Imm, c.FalseBranchSize)}
...
func GuardJump(ins, insClass uint8, dstReg *Register, imm int32) *IMMJMPOperation {
...
func (c *RegJMPOperation) GenerateBytecode() []uint64 {
	bytecode := []uint64{encodeRegisterJmpOperation(c.Instruction, c.InsClass, c.DstReg.RegisterNumber(), c.SrcReg.RegisterNumber(), c.FalseBranchSize)}
	if c.FalseBranchNextInstr != nil {
		// Only take the `c.FalseBranchSize` number of opcodes of the
		// false brach generated bytecode.
		falseBranchBytecode := c.FalseBranchNextInstr.GenerateBytecode()[0:c.FalseBranchSize]
