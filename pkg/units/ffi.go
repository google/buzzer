...
//struct bpf_result ffi_load_ebpf_program(void* prog_buff, size_t size, void* func_buff, void* func_info, size_t func_info_size, int coverage_enabled, unsigned long coverage_size);
...
// settings.
func (e *FFI) ValidateEbpfProgram(prog []uint64, func_info []uint64, func_buff []byte) (*fpb.ValidationResult, error) {
	if len(prog) == 0 {
	...
	bpfVerifyResult := C.ffi_load_ebpf_program(unsafe.Pointer(&prog[0]), C.ulong(len(prog)), unsafe.Pointer(&func_buff[0]), unsafe.Pointer(&func_info[0]), C.ulong(len(func_info)), C.int(cbool) /*coverage_size=*/, C.ulong(coverageSize))
